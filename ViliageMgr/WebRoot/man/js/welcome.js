$(document).ready(function() {
	$("#hzs_select").change(function() {
		...
		if(index == "0") {
			...
			$("#numFarmerName,#numLszUser,#numLszText,#numLszContact,#numDlrUser,#numDlrText,#numDlrContact,#yz_name").hide(); //合作社字段

		} else if(index == "1") {
			$("#numText,#numContact,#numUser,#yz_name").hide(); //农场三个字段
			...
			$("#numText,#numContact,#numUser,#yz_name").show(); //规模化养殖小区的三个字段
...
$("#add_type_one").click(function() {
	...
	//规模化第一种
	$("#num_text,#num_contact,#num_user,#yz_name").removeAttr("disabled");
});
$("#add_type_two").click(function() {
	...
	//规模化第二种
	$("#num_text,#num_contact,#num_user,#yz_name").val("");
	$("#num_text,#num_contact,#num_user").attr("disabled", "disabled");
});
...
function AddUser() {
	...
	//*******************选择合作社的话**********************
		...
		if($("input:radio[name='add_type']:checked").val() == "0") {
			...
			if(farm_name == '' || farmer == "请输入规模化养殖场名称") {
				alert("姓名不能为空");
				return false;
			}
			$("#num_farmer_name,#num_lsz_user,#num_lsz_text,#num_lsz_contact,#num_dlr_user,#num_dlr_text,#num_dlr_contact,#num_file").val("");
			//家庭农场第一种上传方式
			$.post("/ViliageMgr/api/admin/user/addFarm", $("form").serialize(),
				function(data, textStatus) {
					if(textStatus == 'success' && data != null) {
						if(data.code == 0) {
							alert("新增数据成功!");
...
//****选择养殖场小区中的第二种方式————excel导入******
			...
			$('#onSubmit').ajaxSubmit({
				url: "/ViliageMgr/api/admin/user/addFarm?add_type=1",
				success: function(obj) {
					var data = JSON.parse(obj);
					if(data.code == 0) {
						alert("数据导入成功!");
